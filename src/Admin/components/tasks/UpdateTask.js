...
export default function UpdateTask(){
    ...
    useEffect(
        ()=>{
            ...
            ApiServices.singletask({_id : _id}).then(
                x=>{
                    console.log(x)
                    setTaskName(x.data.data.taskname)
                    setDescription(x.data.data.description)
                    // setLastDate(x.data.data.lastDate)
                }
            )
        },[1]
    )
    ...
    const formHandler = (e) =>{
        ...
        ApiServices.updatetask(data).then(
            x=>{
                console.log(x)
                if(x.data.success){
                    ...
                    <form onSubmit={formHandler}>
                        ...
                        <div className="row">
                            <div className="form-group col">
                                ...
                                <input type="text" className="form-control"  value={isComplete} onChange={(e)=>{setIsComplete(e.target.value)}}
                                />
                            </div>
                        </div>
